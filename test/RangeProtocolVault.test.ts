...
let fusionXFactory: IFusionXV3Factory;
let fusionXPool: IFusionXV3Pool;
let token0: IERC20;
let token1: IERC20;
...
describe.only("RangeProtocolVault", () => {
  before(async () => {
    ...
    fusionXFactory = (await ethers.getContractAt(
      "IFusionXV3Factory",
    ...
    await fusionXFactory.createPool(token0.address, token1.address, poolFee);
    fusionXPool = (await ethers.getContractAt(
      "IFusionXV3Pool",
      await fusionXFactory.getPool(token0.address, token1.address, poolFee)
    )) as IFusionXV3Pool;

    await fusionXPool.initialize(encodePriceSqrt("1", "1"));
    await fusionXPool.increaseObservationCardinalityNext("15");
  ...
  it("should not reinitialize the vault", async () => {
    await expect(
      vault.initialize(fusionXFactory.address, 1, "0x")
    ).to.be.revertedWith("Initializable: contract is already initialized");
  });

  it("should not mint when vault is not initialized", async () => {
    await expect(vault.mint(amount0, [0, 0])).to.be.revertedWithCustomError(
      vault,
      "MintNotStarted"
    );
  });
  ...
  it("should mint with zero totalSupply of vault shares", async () => {
    ...
    expect(await token0.balanceOf(fusionXPool.address)).to.be.equal(0);
    expect(await token1.balanceOf(fusionXPool.address)).to.be.equal(0);
    ...
    expect(await token0.balanceOf(fusionXPool.address)).to.be.equal(_amount0);
    expect(await token1.balanceOf(fusionXPool.address)).to.be.equal(_amount1);
  ...
  describe("Remove Liquidity", () => {
    ...
    it("should remove liquidity by manager", async () => {
      ...
      const { _liquidity: liquidityBefore } = await fusionXPool.positions(
      ...
      const { _liquidity: liquidityAfter } = await fusionXPool.positions(
        position(vault.address, lowerTick, upperTick)
      );
    ...
    it("should burn vault shares when liquidity is removed", async () => {
      const { _liquidity: liquidity } = await fusionXPool.positions(
        position(vault.address, lowerTick, upperTick)
      );
  ...
  describe("Add Liquidity", () => {
    ...
    it("should add liquidity by manager", async () => {
      ...
      const { sqrtPriceX96 } = await fusionXPool.slot0();
  ...
  describe("Swap", () => {
    it("should fail when minAmountIn is not satisfied", async () => {
      const { sqrtPriceX96 } = await fusionXPool.slot0();
      const liquidity = await fusionXPool.liquidity();
